...
CREATE TABLE IF NOT EXISTS `mydb`.`User` (
  ...
  `resetPwdEmailLink` CHAR(36) NULL,
  `accteptedPrivacy` TINYINT(1) NOT NULL,
  `lastLoginTimeMillis` BIGINT(20) NULL,
  `keyForFastLogin` VARCHAR(45) NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC),
  UNIQUE INDEX `verify_mail_link_UNIQUE` (`verifyEmailLink` ASC),
  UNIQUE INDEX `resend_mail_link_UNIQUE` (`resetPwdEmailLink` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Product` (
  ...
  FULLTEXT INDEX `fulltext_idx` (`name`, `description`),
