 ...
 *
 * STD USER The case where a std user is trying to modify a product has already
 * been filtered in PrepareAddUpdateProductPageServlet, so if user.isAdmin() ==
 * false, this servlet _creates_ a private product, and links the product to the
 * list, as we know that the only action a std user can be performing is that of
 * creating a private product to add it to a list. In this scenario, listId
 * _must not_ be null. If it's null, it's an error.
 *
 * ADMIN USER In case the user is an admin, we know that if listId parameter is
 * null: the admin is either creating a new product, or updating an existing
 * product. so if a productId parameter != null, it means that a product is
 * being updated if productId parameter == null, a product is being created. if
 * listId parameter != null: the admin is either creating a new product, which
 * will be added to a selected list, or updating an existing product. so if a
 * productId parameter != null, it means that only the action of updating a
 * product will be performed. if a productId parameter == null, it means that a
 * new product will be created, and then added to the list
 *
 * @author luca_morgese
 */
public class AddUpdateProductServlet extends HttpServlet
{
        ...
        protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
        {
                ...
                else
                {
                        ...
                        //se non è un admin, crea un prodotto privato
